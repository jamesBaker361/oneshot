...
def draw_pose_dark(keypoints,img):
    pass
...
def get_pose_pair(pil_img):
    ...
    input = []
    image_rgb = image_bgr[:, :, [2, 1, 0]] #convert to rgb
    img = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB) #fix colors
    img_tensor = torch.from_numpy(img / 255.).permute(2, 0, 1).float().to(device) #make tensor
    input.append(img_tensor)

    pred_boxes = get_person_detection_boxes(box_model, input, threshold=0.9)

    black_img=np.full_like(image_bgr,0.0)
    if len(pred_boxes) >= 1:
